...
Cohervia uses trial-level evidence plus two distinct configuration-level assessment stages.
...
Only the Holdout A configuration-level assessment may assign `positive`, `not_positive`, or `invalid` emergence classification.

### Configuration-level transfer assessment

A `CapabilityTransferConfigurationAssessment` aggregates Holdout B evidence for one Holdout-A-positive configuration. It records the task-family stratum, frozen comparator mapping, target/comparator trial counts, `Delta_transfer_B`, `transfer_delta_min`, uncertainty and multiplicity results, verifier result, protocol validity, transfer classification, evaluator identity, supporting trial IDs, and provenance.

Only this Holdout B record may assign `confirmed`, `not_confirmed`, or `invalid` transfer classification. It is generated by the frozen transfer evaluator without access to Cohervia observer outputs.
...
- warning rate/sensitivity across all trials from configurations that are Holdout-A-positive **and** Holdout-B-transfer-confirmed;
...
- feature preprocessing and normalization parameters are frozen before Holdout A and cannot be updated from Holdout A, Holdout B, or Holdout C trajectories;
...
2. Holdout A trials execute and produce an immutable capability result bundle;
3. the frozen capability evaluator emits configuration-level emergence assessments;
4. the frozen A-to-B transfer rule determines which Holdout-A-positive configurations and capability comparators are eligible for Holdout B;
5. only then may the frozen transfer evaluator open Holdout B;
6. Holdout B trials execute without exposing Cohervia observer outputs to the transfer evaluator;
7. Holdout B results are written to a separate immutable transfer result bundle and the transfer evaluator emits configuration-level transfer assessments;
8. the frozen B-to-C governance-selection rule determines which transfer-confirmed configurations and topology-matched sham controls are eligible for Holdout C;
9. only then may the frozen governance evaluator open Holdout C;
10. Holdout C trials execute with the already-frozen Cohervia observer;
11. Holdout C results are written to a third immutable governance result bundle.

No material component may change between steps 1 and 11.

If a material defect requires a change after Holdout A has been opened, the confirmatory sequence is invalidated and requires new eligible Holdouts A, B, and C under an amended or new preregistration.
